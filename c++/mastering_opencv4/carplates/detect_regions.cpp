...
cv::Mat DetectRegions::histEq(cv::Mat in)
{
    ...
    if (in.channels() == 3) {
        ...
        cv::cvtColor(hsv, out, cv::CV_HSV2BGR);
...
std::vector<Plate> DetectRegions::segment(cv::Mat input)
{
    ...
    cv::threshold(img_sobel, img_threshold, 0, 255, cv::CV_THRESH_OTSU+
                                                    cv::CV_THRESH_BINARY);
    ...
    cv::Mat element = cv::getStructureElement(cv::MORPH_RECT, cv::Size(17, 3));
    cv::morphologyEx(img_threshold, img_threshold, cv::CV_MOP_CLOSE, element);
    if (show_steps) {
        cv::imshow("close", img_threshold);
    }

    std::vector<std::vector<cv::Point> contours;
    cv::findContours(img_threshold, contours, cv::CV_RETR_EXTERNAL, 
    ...
    for (int i = 0; i < rects.size(); i++) {
        ...
        int flags = connectivity + (new_mask_val << 8) + 
            cv::CV_FLOODFILL_FIXED_RANGE + cv::CV_FLOODFILL_MASK_ONLY;
        ...
        cv::RotatedRect min_rect = cv::minAreaRect(point_interest);
